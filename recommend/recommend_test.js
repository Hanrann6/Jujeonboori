...
const client = new recombee.ApiClient(
  "alcohol-dev",
  "vWCsEoArpSs3UDJg334CQbOYZMYbuq7ADtzsFMRLDts79BubNbttq6uU1PcdxLe8",
  {
    region: "eu-west",
    timeout: 15000,
  }
...
async function recommendByUserPref(userId, count = 5, preference = {}) {
  ...
  return result.recomms.map((item) => ({
    ...
    reason: `사용자가 선택한 주류 ${item.values.type} 중 가격 ${item.values.price}의 단맛 ${item.values.sweetness}, 신맛 ${item.values.sourness}, 청량감 ${item.values.sparkling}, 바디감 ${item.values.body}, 도수 ${item.values.abv} 등 유사 수치`,
  }));
}
