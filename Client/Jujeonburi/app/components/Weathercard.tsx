 ...
 * - minute < 45면 이전 시각(…:30), minute >= 45면 현재 시각(…:30)
 * - 자정 직후 처리 포함
 */
...
// 응답 item 중 같은 fcstDate에서 “현재시각에 가장 가까운 fcstTime” 선택 
...
/** ====== KMA 호출 ====== */
async function fetchUltraShortWeather(lat: number, lon: number) {
  ...
  const t1h = items.find(
    (it: any) => it.category === "T1H" && it.fcstDate === fcstDate && it.fcstTime === fcstTime
  );
  const temperature = t1h ? Number(t1h.fcstValue) : null;

  const skyRaw = items.find(
    (it: any) => it.category === "SKY" && it.fcstDate === fcstDate && it.fcstTime === fcstTime
  )?.fcstValue;

  const ptyRaw = items.find(
    (it: any) => it.category === "PTY" && it.fcstDate === fcstDate && it.fcstTime === fcstTime
  )?.fcstValue;

  // 문자열/숫자 혼용 대비: 코드값은 Number로 통일
  const skyCode = skyRaw != null ? Number(skyRaw) : null;
  const ptyCode = ptyRaw != null ? Number(ptyRaw) : null;

  const skyDesc =
    skyCode === 1 ? "맑음" : skyCode === 3 ? "구름많음" : skyCode === 4 ? "흐림" : "";
  const ptyDesc =
    ptyCode === 0 ? "맑음음"
      : ptyCode === 1 ? "비"
      : ptyCode === 2 ? "비/눈"
  ...
  return { temperature, skyDesc, ptyDesc, ptyCode };
}

/** ====== 지역명(표시용) ====== */
async function resolveAreaName(lat: number, lon: number) {
  const useLat = isInKorea(lat, lon) ? lat : SEOUL.lat;
  const useLon = isInKorea(lat, lon) ? lon : SEOUL.lon;
...
export default function Weathercard() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      // 권한 거부/실패 시에 서울로 폴백하도록 처리
      if (status !== "granted") {
        setPlace(await resolveAreaName(SEOUL.lat, SEOUL.lon));
        const { temperature, ptyCode } = await fetchUltraShortWeather(SEOUL.lat, SEOUL.lon);

        setTemp(temperature);

        if (temperature != null) await AsyncStorage.setItem(KEY_TEMP, String(temperature));
        if (ptyCode != null) await AsyncStorage.setItem(KEY_PTY, String(ptyCode));

        //console.log("[Weathercard] saved (fallback seoul):", { temperature, ptyCode });
        return;
      }
      ...
      const { temperature, ptyCode } = await fetchUltraShortWeather(target.lat, target.lon);
      setTemp(temperature);

      if (temperature != null) await AsyncStorage.setItem(KEY_TEMP, String(temperature));
      if (ptyCode != null) await AsyncStorage.setItem(KEY_PTY, String(ptyCode));

    //console.log("[Weathercard] saved:", {temperature,ptyCode,lat: target.lat,lon: target.lon,});
    } catch (e) {
      //예외 시에도 서울 폴백하도록 처리
      setPlace(await resolveAreaName(SEOUL.lat, SEOUL.lon));
      try {
        ...
        //console.log("[Weathercard] saved (error fallback seoul):", { temperature, ptyCode });
      } catch {
        setTemp(null);
      }
    } finally {
